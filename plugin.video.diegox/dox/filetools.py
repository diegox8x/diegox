...
from configuraciones import tools, logger
...
def file_open(path, mode="r"):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        tools.dialog_notification("Error al abrir", path)
        return False
...
def rename(path, new_name, strict=False):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        tools.dialog_notification("Error al renombrar", path)
        return False
    else:
        return True
...
def copy(path, dest, silent=False):
    ...
    try:
        ...
        if fo and fd:
            if not silent:
                dialogo = tools.dialog_progress("Copiando archivo", "")
            size = getsize(path)
            copiado = 0
...
def remove(path):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        tools.dialog_notification("Error al eliminar el archivo", path)
        return False
    else:
        return True
...
def rmdirtree(path):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        tools.dialog_notification("Error al eliminar el directorio", path)
        return False
    else:
...
def mkdir(path):
    ...
    try:
        ...
        logger.error(traceback.format_exc())
        tools.dialog_notification("Error al crear el directorio", path)
        return False
