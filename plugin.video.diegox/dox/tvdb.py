...
from dox import jsontools, scrapertools
from dox.item import InfoLabels
...
DICT_MPAA = {'TV-Y': 'Público pre-infantil: niños menores de 6 años', 'TV-Y7': 'Público infantil: desde 7 años','TV-G': 'Público general: sin supervisión familiar', 'TV-PG': 'Guía paterna: Supervisión paternal','TV-14': 'Mayores de 14 años', 'TV-MA': 'Mayores de 17 años'}
# Traducciones - Fin
...
def find_and_set_infoLabels(item):
    ...
    if not item.contentSeason:
        p_dialog = tools.dialog_progress_bg("Buscando información de la serie", "Espere por favor...")
    ...
    if 'infoLabels' in item and 'season' in item.infoLabels:
        ...
        if item.infoLabels['episode']:
            ...
            if data_episode:
                ...
                thumb = data_episode.get('filename', "")
                if thumb != '':
                    item.thumbnail = HOST_IMAGE + thumb
                    item.infoLabels['episodio_imagen'] = item.thumbnail
            ...
            if data_season and 'image_season_%s' % int_season in data_season:
                ...
                item.infoLabels['thumbnail'] = item.thumbnail
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('tvdb_id', ''):
            ...
            if not self.list_results and config.get_setting('tvdb_retry_eng', default=True):
                ...
                tools.dialog_notification("No se ha encontrado en idioma '%s'" % DEFAULT_LANG, "Se busca en idioma 'en'", sound=False)
            ...
            if not self.list_results and config.get_setting('tvdb_retry_eng', default=True):
                from configuraciones import tools
                tools.dialog_notification("No se ha encontrado en idioma '%s'" % DEFAULT_LANG, "Se busca en idioma 'en'")
