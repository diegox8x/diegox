...
from dox import httptools, proxytoolsz, scrapertools
...
def configurar_proxies_canal(canal, url):
    ...
    if proxies_auto:
        ...
        if proxies_todos:
            ...
            if proxysearch_process == True: pass
               ...
               if proxies_actuales:
                   if proxies_proces:
                       if not tools.dialog_yesno(config.__addon_name + ' - Proxies', 'Existen proxies memorizados en el canal [COLOR yellow][B]' + canal.capitalize() + '[/B][/COLOR]', '[COLOR cyan][B]¿ Desea iniciar una nueva búsqueda de proxies en todos los proveedores ?[/B][/COLOR]'):
                           procesar = False
            ...
            if proxysearch_process == True: pass
               ...
               if proxies_actuales:
                   if proxies_proces:
                       if not tools.dialog_yesno(config.__addon_name, 'Existen proxies memorizados en el canal [COLOR yellow][B]' + canal.capitalize() + '[/B][/COLOR]', '[COLOR cyan][B]¿ Desea iniciar una nueva búsqueda de Proxies con el Proveedor configurado en sus Ajustes categoria Proxies ?[/B][/COLOR] ' + '[COLOR red][B] ' + provider_fijo.capitalize() + '[/COLOR][/B]' ):
                           procesar = False
    ...
    if proxies_finales:
        if not proxies_finales == proxies_iniciales:
            ...
            tools.dialog_notification('Buscar proxies', el_canal)
    ...
    while True:
        ...
        acciones.append(tools.listitem_to_select(texto, lbl, ''))
        acciones.append(tools.listitem_to_select('[COLOR yellow]Buscar nuevos proxies[/COLOR]', 'Buscar con parámetros actuales [COLOR darkcyan][B]Guardará los mejores[/B][/COLOR]'))
        acciones.append(tools.listitem_to_select('[COLOR cyan]Parámetros búsquedas[/COLOR] Proveedor, Tipo, País, ...', '[COLOR goldenrod][B]%s[/B][/COLOR], [COLOR darkorange]%s[/COLOR], [COLOR chocolate]%s[/COLOR], [COLOR darkgoldenrod]%d[/COLOR]' % (provider, tipo_proxy, pais_proxy, max_proxies), ''))

        if proxies: acciones.append(tools.listitem_to_select('[COLOR red]Quitar proxies[/COLOR]', 'Quitar los proxies actuales para probar el canal sin ellos'))

        hay_private = False
        if provider == private_list:
            hay_private = exist_yourlist()
            if hay_private:
                acciones.append(tools.listitem_to_select('[COLOR red]Eliminar lista actual [COLOR cyan] ' + private_list + '[/COLOR]', '[COLOR chocolate][B]Solo si obtuvo una Nueva Lista[/B][/COLOR]'))

        acciones.append(tools.listitem_to_select('[COLOR yellowgreen]Ajustes categoría proxies[/COLOR]', '[COLOR mediumaquamarine]Si los modifica deberá abandonar por Cancelar[/COLOR]'))

        if proxies_help: acciones.append(tools.listitem_to_select('[COLOR green]Ayuda[/COLOR]', 'Información sobre la Gestión de proxies'))

        ret = tools.dialog_select('Configuración proxies para [COLOR darkorange][B]%s[/B][/COLOR]' % canal.capitalize(), acciones, useDetails=True)

        if ret == -1: break

        elif ret == 0:
            new_proxies = tools.dialog_input(default=proxies, heading='Indicar Uno ó varios Proxis a utilizar separados por comas')
            if new_proxies:
                if '.' in new_proxies and ':' in new_proxies: pass
                else:
                    tools.dialog_notification(canal, 'Formato proxy incorrecto')
                    new_proxies = ''
            ...
            if not search_provider:
                if provider == all_providers: search_provider = True
                    ...
                    if not proxies:
                       ...
                       if tools.dialog_yesno(config.__addon_name, '[COLOR cyan][B]¿ Desea iniciar la búsqueda de proxies en todos los proveedores ?[/B][/COLOR]', 'en el Canal [COLOR yellow][B]' + canal.capitalize() + '[/B][/COLOR]'):
                           search_provider = True
...
def _settings_proxies_canal(canal, opciones_provider):
    ...
    if proxies_auto:
        if not proxies_todos:
            ...
            if not provider == provider_fijo:
                if tools.dialog_yesno(config.__addon_name, 'Tiene seleccionado un proveedor que no es el asignado en su configuración de proxies [COLOR cyan]' + provider.capitalize() + '[/COLOR]', '¿ Desea asignar este proveedor para este canal [COLOR yellow][B]' + canal.capitalize() + '[/B][/COLOR] ?'):
    ...
    if not proxies_tipos: ret = 0
        ...
        ret = tools.dialog_select('Tipo de anonimidad de los proxies', opciones_tipo, preselect=preselect)
        if ret == -1: return False
    ...
    if not proxies_paises: ret = 0
        ...
        ret = tools.dialog_select('País de los proxies', opciones_pais, preselect=preselect)
        if ret == -1: return False
    ...
    try:
        max_proxies = int(tools.dialog_numeric(0, 'Valor máximo de proxies a analizar (tope 500)', '50'))
        if ret == -1: return False

        if max_proxies < 3 or max_proxies > 500:
            tools.dialog_notification(canal, 'Valor incorrecto, mínimo 3, máximo 500')
            max_proxies = 50
...
def _buscar_proxies(canal, url, provider, procesar):
    ...
    if extended:
        if tplus == True:
            if provider == 'z-tplus':
                ...
                tools.dialog_notification('Buscar en Tplus', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-echolink':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Echolink', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-free-proxy-list.uk':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Free-proxy-list-uk', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-free-proxy-list.anon':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Free-proxy-list-anon', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-github':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Github', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-opsxcq':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Opsxcq', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-proxy-daily':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Proxy-daily', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-proxy-list.org':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Proxy-list.org', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-proxyhub':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Proxhub', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-proxyranker':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Proxyranker', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-squidproxyserver':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Squidproxyserver', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-socks':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Socks', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-free-proxy-list.com':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Free-proxy-list-com', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-xroxy':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Xroxy', msg_txt % color_infor)
        ...
        if search_provider or provider == 'z-coderduck':
            ...
            if searching:
                if len(all_providers_proxies) < proxies_totales_limit:
                    if search_provider: tools.dialog_notification('Buscar en Coderduck', msg_txt % color_infor)
    ...
    if search_provider or provider == 'mmpx12':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Mmpx12', msg_txt % color_infor)
    ...
    if search_provider or provider == default_provider:
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxyscrape', msg_txt % color_infor)
    ...
    if search_provider or provider == 'us-proxy.org':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Us-proxy', msg_txt % color_infor)
    ...
    if search_provider or provider == 'almroot':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Almroot', msg_txt % color_infor)
    ...
    if search_provider or provider == 'free-proxy-list':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Free-proxy-list', msg_txt % color_infor)
    ...
    if search_provider or provider == 'google-proxy.net':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Google-proxy', msg_txt % color_infor)
    ...
    if search_provider or provider == 'hidemy.name':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Hidemy-name', msg_txt % color_infor)
    ...
    if search_provider or provider == 'ip-adress.com':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Ip-adress', msg_txt % color_infor)
    ...
    if search_provider or provider == 'dailyproxylists.com':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Dailyproxylists', msg_txt % color_infor)
    ...
    if search_provider or provider == 'proxysource.org':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxysource', msg_txt % color_infor)
    ...
    if search_provider or provider == 'spys.one':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Spys-one', msg_txt % color_infor)
    ...
    if search_provider or provider == 'mertguvencli':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Mertguvencli', msg_txt % color_infor)
    ...
    if search_provider or provider == 'shiftytr':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Shiftytr', msg_txt % color_infor)
    ...
    if search_provider or provider == 'roosterkid':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Roosterkid', msg_txt % color_infor)
    ...
    if search_provider or provider == 'clarketm':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Clarketm', msg_txt % color_infor)
    ...
    if search_provider or provider == 'sslproxies.org':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Sslproxies', msg_txt % color_infor)
    ...
    if search_provider or provider == 'httptunnel.ge':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Httptunnel', msg_txt % color_infor)
    ...
    if search_provider or provider == 'geonode.com':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Geonode', msg_txt % color_infor)
    ...
    if search_provider or provider == 'proxy-list.download':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxy-list', msg_txt % color_infor)
    ...
    if search_provider or provider == 'spys.me':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Spys-me', msg_txt % color_infor)
    ...
    if search_provider or provider == 'proxynova.com':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxynova', msg_txt % color_infor)
    ...
    if search_provider or provider == 'proxyservers.pro':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxyservers', msg_txt % color_infor)
    ...
    if search_provider or provider == 'silverproxy.xyz':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Silverproxy', msg_txt % color_infor)
    ...
    if search_provider or provider == 'proxydb.net':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Proxydb', msg_txt % color_infor)
    ...
    if search_provider or provider == 'hidester.com':
        ...
        if searching:
            if len(all_providers_proxies) < proxies_totales_limit:
                if search_provider: tools.dialog_notification('Buscar en Hidester', msg_txt % color_infor)
    ...
    if provider == private_list: proxies = obtener_private_list()
       ...
       if not provider:
           if not search_provider:
               tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Parámetros desconocidos[/COLOR][/B]' % color_alert)
               return False

    if not all_providers_proxies:
        if not proxies:
            if providers_preferred:
                tools.dialog_ok('Buscar proxies ' + provider.capitalize(), 'Tiene informados en Ajustes [COLOR wheat][B]Proveedores Preferidos[/B][/COLOR] de proxies.', '[COLOR yellow][B]Sin proxies según sus parámetros actuales.[/B][/COLOR]', '[COLOR red]Preferidos: [COLOR violet][B]' + str(providers_preferred + '[/B][/COLOR]'))
            else:
                tools.dialog_notification('Buscar proxies ' + provider.capitalize(), '[B][COLOR %s]Sin proxies según parámetros[/COLOR][/B]' % color_adver)
            return False
    ...
    if search_provider:
        ...
        if len(proxies) >= 50: tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Cargando proxies ...[/COLOR][/B]' % color_adver)
    ...
    if len(selected) > 0:
        ...
        if not proxies_actuales:
            ...
            if avisar: tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Sin proxies válidos[/COLOR][/B]' % color_alert)
 
        else:
            if provider == private_list: tools.dialog_notification('Buscar proxies', 'Sin proxies válidos [B][COLOR %s]en su lista[/COLOR][/B]' % color_alert)
            else:
               ...
               tools.dialog_ok('Búsqueda proxies en [COLOR red][B]' + provider.capitalize() + '[/B][/COLOR]', '[COLOR yellow][B]Sin proxies validos.[/B][/COLOR]', texto_mensaje, '[COLOR coral][B]Seleccione otro Proveedor en los [COLOR cyan][B]Parámetros de Búsquedas.[/B][/COLOR]')
...
def sin_news_proxies(provider, proxies_actuales, procesar):
    ...
    if avisar:
        ...
        tools.dialog_ok('Búsqueda proxies en [COLOR red][B]' + provider.capitalize() + '[/B][/COLOR]', '[COLOR yellow][B]No se obtuvo ningún proxy válido con este Proveedor.[/B][/COLOR]', texto_mensaje, '[COLOR coral][B]Puede intentar obtener nuevos Proxies, cambiando de Proveedor, en los Parámetros para Buscar Proxies.[/B][/COLOR]')
...
def top_news_proxies(provider, proxies_actuales, valor, procesar):
    ...
    if avisar:
        ...
        tools.dialog_ok('Búsqueda proxies en [COLOR red][B]' + provider.capitalize() + '[/B][/COLOR]', '[COLOR yellow][B]Se han obtenido ' + str(valor) + ' proxies válidos con este Proveedor.[/B][/COLOR]', texto_mensaje, '[COLOR coral][B]Puede intentar obtener menos Proxies, cambiando de Proveedor, en los Parámetros para Buscar Proxies.[/B][/COLOR]')
...
def _dailyproxylists_com(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if enlaces:
        ...
        tools.dialog_notification('Dailyproxylists.com', 'Vía' + el_provider)
...
def _mertguvencli(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if enlaces:
        ...
        tools.dialog_notification('Tplus', 'Vía' + el_provider)
...
def _spys_one(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if '<title>Just a moment...</title>' in resp.data or '<title>Just a moment please...</title>' in resp.data:
        ...
        tools.dialog_notification('Spys.one', 'Vía' + el_provider)
...
def _hidemy_name(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if '<title>Just a moment...</title>' in resp.data or '<title>Just a moment please...</title>' in resp.data:
        ...
        if enlaces:
            ...
            tools.dialog_notification('Hidemy.name', 'Vía' + el_provider)
...
def _httptunnel_ge(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if enlaces:
        ...
        tools.dialog_notification('Httptunnel', 'Vía' + el_provider)
...
def _silverproxy_xyz(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    tools.dialog_notification('Silverproxy', 'Vía' + el_provider)
...
def _proxyscrape_com(url, tipo_proxy, pais_proxy, max_proxies):
    ...
    if not resp.data: 
        ...
        tools.dialog_notification('Proxyscrape', '[COLOR cyan]Re-Intentanto acceso[/COLOR]')
...
def testear_lista_proxies(canal, provider, url, proxies=[]):
    ...
    progreso = tools.dialog_progress('Test proxies ' + '[COLOR yellow][B]' + canal.capitalize() + '[/B][/COLOR] con [COLOR red][B]' + txt_provider + '[/B][/COLOR]', '%d proxies a comprobar. [COLOR yellowgreen][B]Cancelar si tarda demasiado[/B][/COLOR].' % num_proxies)
    ...
    for proxy in proxies:
        ...
        if proxy in proxies_info:
            if repeated == 0:
                repeated += 1
                tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Despreciando repetidos[/COLOR][/B]' % color_avis)
            continue
    ...
    if not proceso_test:
        if tools.dialog_yesno('ERROR Test proxies en [COLOR yellow][B]' + provider.capitalize() + '[/B][/COLOR]', '[COLOR red][B]Sin disponibilidad de suficiente Memoria para este proceso.[/B][/COLOR]', '[COLOR yellow][B]¿ Desea anular el test automatico en TODOS los proveedores para intentar evitar este inconveniente ?[/B][/COLOR]'):
...
def show_help_proxies():
    ...
    if providers_preferred:
        tools.dialog_ok(config.__addon_name + ' - Ajustes', 'Tiene informados en sus Ajustes [COLOR cyan]Proveedores Preferidos[/COLOR] de Proxies.')
...
def configuracion_general():
    ...
    tools.dialog_ok(config.__addon_name + ' - Ajustes', '[COLOR yellow][B]Si efectuó alguna variación en sus Ajustes de Proxies.[/COLOR][/B]', '[COLOR cyan][B]Recuerde, que para que los [COLOR gold]Cambios Surtan Efecto[/COLOR][COLOR cyan], deberá Abandonar el proceso de Configurar Proxies e ingresar de nuevo en el.[/B][/COLOR]')
...
def obtener_private_list():
    ...
    if not existe:
        if not tools.dialog_yesno(config.__addon_name, '[COLOR violet][B]¿ El fichero [COLOR cyan]Lista-proxies.zip[/COLOR][COLOR violet] ya está Des-comprimido ?[/COLOR][/B]'): 
            ...
            if not existe_zip:
                tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Fichero .ZIP No localizado[/COLOR][/B]' % color_alert)
                time.sleep(1.0)
                return proxies
        ...
        if not existe:
            tools.dialog_notification('Buscar proxies', '[B][COLOR %s]Fichero Privado No localizado[/COLOR][/B]' % color_alert)
            time.sleep(1.0)
            return proxies

        if tools.dialog_yesno(config.__addon_name, '[COLOR cyan][B]¿ Desea Guardar una copia del fichero [COLOR yellow]Lista-proxies.txt[COLOR cyan] en la carpeta por defecto [COLOR chocolate].../addon_data.../plugin.video.balandro[/B][/COLOR] ?'): 
            proxies_file = os.path.join(config.get_data_path(), private_list)
            ...
            if not filetools.copy(origen, destino, silent=False):
                tools.dialog_ok(config.__addon_name + ' - Lista-proxies.txt', '[COLOR red][B]Error, no se ha podido copiar el fichero[/B][/COLOR]', origen, destino)
                return proxies
            else:
                tools.dialog_notification('Fichero copiado', proxies_file)

                if zip_extract:
                    if tools.dialog_yesno(config.__addon_name, '[COLOR red][B]¿ Desea Eliminar el fichero [COLOR yellow]Lista-proxies.txt [COLOR red]ya [COLOR yellow]Copiado[COLOR red] ?[/B][/COLOR]'):
                        filetools.remove(origen)
                        tools.dialog_notification(config.__addon_name, '[B][COLOR %s]Fichero Lista-proxies.zip eliminado[/B][/COLOR]' % color_exec)

                    if tools.dialog_yesno(config.__addon_name + ' - ZIP DESCARGADO', '[COLOR red][B]¿ Desea Eliminar el fichero [COLOR cyan]Lista-proxies.zip [COLOR red]descargado ?[/B][/COLOR]'):
